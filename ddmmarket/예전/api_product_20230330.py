...
# call(http://ip:port/call?store_id=CB1NA24367)
...
def model_predict(store_id):
    ...
    # =========== 매장 ===========
    for c_link in tqdm(crawling_link_list, desc='[Crawling tqdm] 매장별 링크', position=0):
        driver.get(c_link)
        driver.implicitly_wait(10)
        time.sleep(5)
        ...
        try:
            # 상품 수
            element = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, 'totalItem')))
            all_product_num = int(driver.find_element(By.ID, 'totalItem').text)
        
        except:
            print("[ERROR] 상품 수 미인식")
            all_product_num = 0
        ...
        for cate_len_idx in range(category_len):
            ...
            try:
                ...
                # print(all_product_num, product_len)
            ...
            for product_len_idx in tqdm(range(product_len), desc='[Crawling tqdm] 카테고리별 상품들', position=1, leave=False):
                try:
                    ...
                    df_product_one = pd.DataFrame({
                        ...
                        'all_product_num': [all_product_num],
    ...
    # load
    # df_all = pd.read_csv(f'./shop_total/{shop_name}_product_list.csv')

    df_all.loc[df_all['product_fitting']=='베이직', 'product_fitting'] = '기본핏'
    df_all.loc[df_all['product_seethrough']=='약간비침', 'product_seethrough'] = '약간'
    df_all.loc[df_all['product_seethrough']=='비침', 'product_seethrough'] = '있음'
    df_all = df_all.replace('미표기', '')
    ...
    for d in d_shop_name:
        for s in rows_shops:
            ...
            table_shops = (
                ...
                0, # is_recommend
                None, # sinsang_거래처수,
                df_all[df_all['shop_name'] == d]['all_product_num'].value_counts().tolist()[0], # sinsang_전체상품수
                None, # sinsang_거래처상품수,
                None, # partners_count
...
def exception_handler(error):
    driver.quit()
    print(error)
    log_message = {'Message':'Fail Insert DB & Call IP : {request.remote_addr}'}
    return log_message
