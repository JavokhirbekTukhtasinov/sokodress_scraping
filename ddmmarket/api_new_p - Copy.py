...
def job():
    ...
    folder_products_image = 'products_image'
    folder_shops = 'shops'
    ...
    for c_link in tqdm(shops_link_list, desc='[Crawling tqdm] 매장별 링크', position=0, leave=True):
        ...
        for cate_len_idx in range(category_len):
            ...
            try:
                ...
                print(f"체크박스 체크 후, 상품 X pass OR 지연 스킵 에러")
                continue
            ...
            for product_len_idx in tqdm(range(product_len), desc='[Crawling tqdm] 카테고리별 상품들', position=1, leave=False):
                try:
                    ...
                    if SHOPS_HOLD_SWITCH:
                        product_category_2 = shops_hold_category[0]
                        product_category_3 = shops_hold_category[1]
        ...
        df_product.to_csv(f'./{folder_backup}/{folder_shops}/{shop_name}.csv', index=False)
